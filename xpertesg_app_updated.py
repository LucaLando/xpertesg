    ...
    if aba == " Clientes":
        ...
        with subtab1:
            ...
            """
                ...
                st.error(f"Erro no Chat com Fábio: {e}")

        with subtab2:
            st.header("Portal Informações ESG")
            if st.session_state.api_key:
                openai.api_key = st.session_state.api_key
                prompt_text = '''Faça uma análise comparativa da rentabilidade média líquida dos produtos ESG comercializados na plataforma da XP desde 2023 em comparação aos investimentos tradicionais, considerando custo de oportunidade e CAPM.'''
                try:
                    ...
                    st.error(f"Erro ao buscar informações ESG: {e}")
            else:
                st.info("Por favor, insira sua API Key na aba Conversa para visualizar o Portal.")
