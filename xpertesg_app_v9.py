...
if st.session_state.usuario:
    ...
    aba = st.sidebar.radio("Navegar para:", [
        "👥 Clientes", "🗣️ Chat com o Fábio", "📦 Produtos ESG",
        "📈 Dashboards", "📌 Recomendações", "💡 Alocação Inteligente"
    ])
        ...
        if st.button("Enviar") and prompt_usuario and st.session_state.api_key:
            ...
            try:
                resposta = client.chat.completions.create(
                    model="gpt-4",
                    messages=[
                        {"role": "system", "content": "Você é o Fábio, especialista ESG da XP Inc."}
                    ] + st.session_state.mensagens
                )
...
# As demais abas foram omitidas aqui por limite de espaço, mas seguem o padrão anterior.
